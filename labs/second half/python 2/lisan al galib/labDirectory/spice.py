...
### Return : np array of size Kx2
def update_centerss(data, labels,n,new_centers):
    cluster_points = data[labels == n] 
    new_centers[n] = np.mean(cluster_points, axis=0)
    if(n>0) :
        update_centerss(data, labels,n-1,new_centers) 

def update_centers(data, labels, K):
    new_centers = np.zeros((K, 2))
    k=K-1
    update_centerss(data,labels,k,new_centers)
    return new_centers
