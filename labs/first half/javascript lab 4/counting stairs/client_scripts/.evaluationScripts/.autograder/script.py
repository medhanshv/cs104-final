...
data = []
test_case_num = 7
all_test_inputs = [1, 2, 5, 10, 15, 20]
all_test_outputs = [1, 2, 8, 89, 987, 10946]

# ------------------------------test cases---------------------------
current_directory = os.getcwd()
file_url = "file://" + os.path.join(current_directory, "script.html")
...
for i in range(0, test_case_num):
    ...
    input_field = driver.find_element("id", "inputN")
    input_field.clear()
    input_field.send_keys(str(input_number))
    input_field.send_keys(Keys.RETURN)
    button = driver.find_element("xpath", "//button[text()='Calculate']")
    button.click()
    # time.sleep(1)
